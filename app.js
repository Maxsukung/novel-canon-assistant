...
function novelExcerpt(text){return String(text||'').replace(/\[หน้า\s*\d+\]/g,' ').replace(/\s+/g,' ').trim().slice(0,180)}
function renderNovelContent(p){const q=String($('novelSearch')?.value||'').trim().toLowerCase();const docs=(p?.documents||[]).filter(d=>d.type==='chapter').filter(d=>!q||String(d.name||'').toLowerCase().includes(q)||String(d.text||'').toLowerCase().includes(q)).sort((a,b)=>chapterSortNumber(a)-chapterSortNumber(b)||String(a.name).localeCompare(String(b.name),'th'));if($('novelCount'))$('novelCount').textContent=`${docs.length.toLocaleString('th-TH')} บท/ตอน`;$('novelChapterList').innerHTML=docs.length?docs.map((d,i)=>`<article class="novel-chapter-card" data-novel-doc="${d.id}" tabindex="0"><div class="novel-chapter-number">${chapterSortNumber(d)<999999?chapterSortNumber(d):i+1}</div><div class="novel-chapter-main"><span class="badge">ต้นฉบับตอน</span><h3>${esc(d.name.replace(/\.(pdf|docx|txt|md)$/i,''))}</h3><p>${esc(novelExcerpt(d.text)||'ไม่มีข้อความที่อ่านได้')}</p><div class="meta">${(d.text?.length||0).toLocaleString('th-TH')} ตัวอักษร${d.pageCount?` · ${d.pageCount} หน้า`:''}</div></div><button class="view-novel-btn" data-open-novel="${d.id}">อ่าน</button></article>`).join(''):'<div class="empty">ยังไม่มีต้นฉบับตอน กรุณานำเข้าไฟล์ในหน้าเอกสารโดยเลือก “ต้นฉบับตอน”</div>';document.querySelectorAll('[data-open-novel]').forEach(b=>b.onclick=e=>{e.stopPropagation();openNovelReader(b.dataset.openNovel)});document.querySelectorAll('[data-novel-doc]').forEach(card=>{card.onclick=()=>openNovelReader(card.dataset.novelDoc);card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openNovelReader(card.dataset.novelDoc)}}})}
function renderNovelReader(doc){if(!doc){$('novelReaderTitle').textContent='ไม่พบต้นฉบับ';$('novelReaderMeta').textContent='';$('novelReaderBody').innerHTML='<div class="empty">ไฟล์นี้อาจถูกลบแล้ว</div>';return}$('novelReaderType').textContent=documentTypeLabel(doc.type);$('novelReaderTitle').textContent=doc.name.replace(/\.(pdf|docx|txt|md)$/i,'');$('novelReaderMeta').textContent=`${(doc.text?.length||0).toLocaleString('th-TH')} ตัวอักษร${doc.pageCount?` · ${doc.pageCount} หน้า`:''} · นำเข้า ${new Date(doc.createdAt).toLocaleString('th-TH')}`;const raw=String(doc.text||'').trim();if(!raw){$('novelReaderBody').innerHTML='<div class="empty">ไม่พบข้อความที่อ่านได้จากไฟล์นี้</div>';return}const pageParts=raw.split(/(?=\[หน้า\s*\d+\])/g).filter(Boolean);if(pageParts.length>1||/^\[หน้า\s*\d+\]/.test(raw)){$('novelReaderBody').innerHTML=pageParts.map(part=>{const m=part.match(/^\[หน้า\s*(\d+)\]\s*/);const text=part.replace(/^\[หน้า\s*\d+\]\s*/,'').trim();return `<section class="novel-page"><h3>${m?`หน้า ${m[1]}`:'เนื้อหา'}</h3><div class="novel-prose">${esc(text)}</div></section>`}).join('')}else{const paras=raw.split(/\n\s*\n|\n(?=\s{0,3}[^\n]{1,80}(?:\n|$))/).map(x=>x.trim()).filter(Boolean);$('novelReaderBody').innerHTML=`<section class="novel-page"><div class="novel-prose">${paras.map(x=>`<p>${esc(x).replace(/\n/g,'<br>')}</p>`).join('')}</div></section>`}}
