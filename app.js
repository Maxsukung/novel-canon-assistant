...
// ---- Character intelligence from novel text (V29 precision + bulk review) ----
const PERSON_ACTION_WORDS=['กล่าว','พูด','ถาม','ตอบ','ตะโกน','กระซิบ','เรียก','ร้อง','ยิ้ม','หัวเราะ','พยักหน้า','ส่ายหน้า','มอง','เดิน','วิ่ง','หัน','ลุก','นั่ง','ยืน','ถอนหายใจ','เอ่ย','ก้าว','ชะงัก','พึมพำ','สวน','ร้องเรียก'];
const CANDIDATE_STOP_WORDS=new Set(['เขา','เธอ','มัน','เด็กหนุ่ม','เด็กสาว','ชายหนุ่ม','หญิงสาว','ชายชรา','หญิงชรา','ชายคนนั้น','หญิงคนนั้น','คนหนึ่ง','ทุกคน','ไม่มีใคร','ใคร','แม่','พ่อ','พี่','น้อง','ท่าน','พระเอก','นางเอก','บท','ตอน','ป่า','เมือง','โลก','เสียง','แสง','เงา','ความจริง','เวลา','วันนี้','วันนั้น','ถอยออกมา','ถึงแล้ว','พูดมา','ช่วยกัน','อยู่ที่ไหน','อนันต์ดึง','อนันต์พยายาม','อนันต์เพ่ง']);
const THAI_VERB_PREFIXES=['ถอย','ดึง','พยายาม','เพ่ง','พูด','ถาม','ตอบ','ช่วย','อยู่','ถึง','มอง','เดิน','วิ่ง','หัน','ลุก','นั่ง','ยืน','ก้าว','ชะงัก','ยก','วาง','หยุด','รีบ','ค่อย','เริ่ม','กลับ','ออก','เข้า','ลง','ขึ้น','เปิด','ปิด','ร้อง','เรียก','กล่าว','เอ่ย','คิด','รู้สึก','เห็น','ได้ยิน','ต้องการ','สามารถ'];
...
function cleanCandidateName(value){return repairExtractedThaiName(String(value||'')).replace(/^[“”"'‘’\s]+|[“”"'‘’.,!?…:;\s]+$/g,'').replace(/^(?:นาย|นาง|นางสาว|เด็กชาย|เด็กหญิง|เจ้าหญิง|เจ้าชาย|องค์|พญา|ฤๅษี|หลวงตา|ท่าน)\s*/,'').trim()}
function isKnownOrEmbeddedCharacter(name,p){const key=normalizedName(name);if(!key)return true;for(const known of knownCharacterNames(p)){const k=normalizedName(known);if(!k)continue;if(key===k)return true;/* อนันต์ดึง -> อนันต์ already known */if(key.startsWith(k)&&key.length-k.length<=10)return true;if(k.startsWith(key)&&k.length-key.length<=18)return true}return false}
function plausibleNovelCandidate(name){const n=cleanCandidateName(name);if(!n||n.length<2||n.length>18)return false;if(CANDIDATE_STOP_WORDS.has(n))return false;if(/\s{2,}|\d|[A-Za-z]{2,}|(?:บท|ตอน)ที่|DATABASE|CANON|ARC|PART|STATUS/i.test(n))return false;if(/^(?:และ|หรือ|แต่|เมื่อ|เพราะ|ก่อน|หลัง|แล้ว|จึง|กำลัง|ต้องการ|สามารถ|เป็น|มี|ไม่มี|ไม่|การ|ความ|ผู้|สิ่ง|คืนนี้|คราวนี้|ทันใด|จากนั้น)/.test(n))return false;if(THAI_VERB_PREFIXES.some(v=>n===v||n.startsWith(v)))return false;return (n.match(/[ก-ฮ]/g)||[]).length>=2}
function candidateContextScore(text,name){
  const e=String(name||'').replace(/[.*+?^${}()|[\]\\]/g,'\\$&');let score=0;
  for(const w of PERSON_ACTION_WORDS){score+=(text.match(new RegExp(`${e}\\s*(?:${w})`,'g'))||[]).length*5;score+=(text.match(new RegExp(`(?:${w})\\s*(?:ของ\\s*)?${e}`,'g'))||[]).length*2}
  score+=(text.match(new RegExp(`[“"]${e}[”"]`,'g'))||[]).length;return score;
}
function discoverCharacterCandidates(p){
  ...
  for(const chapter of novelEntries(p)){
    ...
    /* Require a compact Thai token directly before a person-action verb. Quoted phrases are no longer candidates. */
    const re=/([ก-ฮ][ก-์]{1,17})\s*(?=กล่าว|พูด|ถาม|ตอบ|ตะโกน|กระซิบ|เรียก|ร้อง|ยิ้ม|หัวเราะ|พยักหน้า|ส่ายหน้า|มอง|เดิน|วิ่ง|หัน|ลุก|นั่ง|ยืน|ถอนหายใจ|เอ่ย|ก้าว|ชะงัก|พึมพำ)/g;
    let m;while((m=re.exec(text))){const name=cleanCandidateName(m[1]),key=normalizedName(name);if(!plausibleNovelCandidate(name)||isKnownOrEmbeddedCharacter(name,p)||ignored.has(key))continue;const row=map.get(key)||{id:uid(),name,chapterIds:new Set(),chapterTitles:new Set(),rawHits:0,contexts:[],confidence:0};row.chapterIds.add(chapter.id);row.chapterTitles.add(chapterDisplayTitle(chapter));row.rawHits++;if(row.contexts.length<3)row.contexts.push(text.slice(Math.max(0,m.index-50),Math.min(text.length,m.index+name.length+90)).replace(/\s+/g,' ').trim());map.set(key,row);if(re.lastIndex===m.index)re.lastIndex++}
  }
  const out=[];for(const row of map.values()){let totalHits=0,ctxScore=0;for(const ch of novelEntries(p)){const hit=countNameHits(ch.text||'',row.name);totalHits+=hit.count;ctxScore+=candidateContextScore(ch.text||'',row.name)}row.count=totalHits;row.chapterIds=[...row.chapterIds];row.chapterTitles=[...row.chapterTitles];row.confidence=Math.min(99,Math.round(38+Math.min(22,row.rawHits*6)+Math.min(24,row.chapterIds.length*8)+Math.min(15,ctxScore)));
    /* Precision first: one-chapter fragments never enter review; require strong repeated person contexts. */
    if(row.chapterIds.length>=2&&row.rawHits>=2&&ctxScore>=8&&row.confidence>=78)out.push(row)
  }
  return out.sort((a,b)=>b.confidence-a.confidence||b.count-a.count).slice(0,40)
}
async function scanNovelCharacters(){if(!ensureProject())return;const p=project();p.characterCandidates=discoverCharacterCandidates(p);activity('character',`วิเคราะห์ตัวละครจากนิยาย พบ ${p.characterCandidates.length} รายการรอตรวจ (โหมดแม่นยำ)`);await save();toast(p.characterCandidates.length?`พบผู้สมัครที่ผ่านเกณฑ์ ${p.characterCandidates.length} รายการ`:'ไม่พบชื่อใหม่ที่ผ่านเกณฑ์')}
async function acceptCharacterCandidate(id){const p=project(),c=(p.characterCandidates||[]).find(x=>x.id===id);if(!c)return;if(!isKnownOrEmbeddedCharacter(c.name,p))p.characters.push({id:uid(),name:c.name,status:'ไม่ทราบ',role:'พบจากเนื้อหานิยาย',aliases:[],facts:`พบใน ${c.chapterTitles.length} ตอน รวม ${c.count} ครั้ง`,limits:'',structured:{'ปรากฏครั้งแรก':c.chapterTitles[0]||''},source:'สกัดจากเนื้อหานิยาย',autoExtractedFromNovel:true,updatedAt:now()});p.characterCandidates=p.characterCandidates.filter(x=>x.id!==id);await save();toast(`เพิ่ม ${c.name} เป็นตัวละครแล้ว`)}
