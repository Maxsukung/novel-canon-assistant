...
// ---- Character intelligence from novel text (V28 foundation) ----
const PERSON_ACTION_WORDS=['กล่าว','พูด','ถาม','ตอบ','ตะโกน','กระซิบ','เรียก','ร้อง','ยิ้ม','หัวเราะ','พยักหน้า','ส่ายหน้า','มอง','เดิน','วิ่ง','หัน','ลุก','นั่ง','ยืน','ถอนหายใจ','เอ่ย'];
const CANDIDATE_STOP_WORDS=new Set(['เขา','เธอ','มัน','เด็กหนุ่ม','เด็กสาว','ชายหนุ่ม','หญิงสาว','ชายชรา','หญิงชรา','ชายคนนั้น','หญิงคนนั้น','คนหนึ่ง','ทุกคน','ไม่มีใคร','ใคร','แม่','พ่อ','พี่','น้อง','ท่าน','พระเอก','นางเอก','บท','ตอน','ป่า','เมือง','โลก','เสียง','แสง','เงา','ความจริง','เวลา','วันนี้','วันนั้น']);
function knownCharacterNameSet(p){const set=new Set();for(const c of p?.characters||[])for(const n of characterAliasCandidates(c))set.add(normalizedName(n));return set}
...
function plausibleNovelCandidate(name){const n=cleanCandidateName(name);if(!n||n.length<2||n.length>28)return false;if(CANDIDATE_STOP_WORDS.has(n))return false;if(/\d|[A-Za-z]{2,}|(?:บท|ตอน)ที่|DATABASE|CANON|ARC|PART|STATUS/i.test(n))return false;if(/^(?:และ|หรือ|แต่|เมื่อ|เพราะ|ก่อน|หลัง|แล้ว|จึง|กำลัง|ต้องการ|สามารถ|เป็น|มี|ไม่มี|ไม่|การ|ความ|ผู้|สิ่ง)/.test(n))return false;return (n.match(/[ก-ฮ]/g)||[]).length>=2}
function candidateContextScore(text,name){
  const e=String(name||'').replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  let score=0;
  for(const w of PERSON_ACTION_WORDS){
    const re1=new RegExp(`${e}\\s*(?:${w})`,'g');
    const re2=new RegExp(`(?:${w})\\s*(?:ของ\\s*)?${e}`,'g');
    score+=(text.match(re1)||[]).length*4+(text.match(re2)||[]).length*3;
  }
  score+=(text.match(new RegExp(`[“"]${e}[”"]`,'g'))||[]).length*2;
  return score;
}
function discoverCharacterCandidates(p){const known=knownCharacterNameSet(p),ignored=new Set((p.ignoredCharacterNames||[]).map(normalizedName)),map=new Map();for(const chapter of novelEntries(p)){const text=String(chapter.text||'');const patterns=[/([ก-ฮ][ก-์]{1,24})\s*(?=กล่าว|พูด|ถาม|ตอบ|ตะโกน|กระซิบ|เรียก|ร้อง|ยิ้ม|หัวเราะ|พยักหน้า|ส่ายหน้า|มอง|เดิน|วิ่ง|หัน|ลุก|นั่ง|ยืน|ถอนหายใจ|เอ่ย)/g,/[“"]([ก-ฮ][ก-์]{1,24})[”"]/g];for(const re of patterns){let m;while((m=re.exec(text))){const name=cleanCandidateName(m[1]);const key=normalizedName(name);if(!plausibleNovelCandidate(name)||known.has(key)||ignored.has(key))continue;const row=map.get(key)||{id:uid(),name,chapterIds:new Set(),chapterTitles:new Set(),count:0,contexts:[],confidence:0};row.chapterIds.add(chapter.id);row.chapterTitles.add(chapterDisplayTitle(chapter));row.count++;if(row.contexts.length<4)row.contexts.push(text.slice(Math.max(0,m.index-55),Math.min(text.length,m.index+name.length+100)).replace(/\s+/g,' ').trim());map.set(key,row);if(re.lastIndex===m.index)re.lastIndex++}}}
  const out=[];for(const row of map.values()){let totalHits=0,ctxScore=0;for(const ch of novelEntries(p)){const hit=countNameHits(ch.text||'',row.name);totalHits+=hit.count;ctxScore+=candidateContextScore(ch.text||'',row.name)}row.count=totalHits;row.confidence=Math.min(99,Math.round(35+Math.min(30,totalHits*3)+Math.min(24,row.chapterIds.size*8)+Math.min(10,ctxScore)));row.chapterIds=[...row.chapterIds];row.chapterTitles=[...row.chapterTitles];if(row.count>=2||row.chapterIds.length>=2||row.confidence>=70)out.push(row)}return out.sort((a,b)=>b.confidence-a.confidence||b.count-a.count)}
async function scanNovelCharacters(){if(!ensureProject())return;const p=project();p.characterCandidates=discoverCharacterCandidates(p);activity('character',`วิเคราะห์ตัวละครจากนิยาย พบ ${p.characterCandidates.length} รายการรอตรวจ`);await save();toast(`พบชื่อที่อาจเป็นตัวละคร ${p.characterCandidates.length} รายการ`)}
async function acceptCharacterCandidate(id){const p=project(),c=(p.characterCandidates||[]).find(x=>x.id===id);if(!c)return;const existing=p.characters.find(x=>normalizedName(x.name)===normalizedName(c.name));if(!existing)p.characters.push({id:uid(),name:c.name,status:'ไม่ทราบ',role:'พบจากเนื้อหานิยาย',aliases:[],facts:`พบใน ${c.chapterTitles.length} ตอน รวม ${c.count} ครั้ง`,limits:'',structured:{'ปรากฏครั้งแรก':c.chapterTitles[0]||''},source:'สกัดจากเนื้อหานิยาย',autoExtractedFromNovel:true,updatedAt:now()});p.characterCandidates=p.characterCandidates.filter(x=>x.id!==id);await save();toast(`เพิ่ม ${c.name} เป็นตัวละครแล้ว`)}
