...
function characterRecordRichness(c){return (String(c.name||'').includes(' ')?30:0)+(c.autoExtracted?15:0)+Object.keys(c.structured||{}).length*3+String(c.facts||'').length/120+String(c.role||'').length/20}
function mergeCharacterRecordInto(target,source){
  const targetName=repairThaiText(target.name||'').trim(),sourceName=repairThaiText(source.name||'').trim();
  const preferred=[targetName,sourceName].sort((a,b)=>{const aw=a.split(/\s+/).length,bw=b.split(/\s+/).length;if(aw!==bw)return bw-aw;return b.length-a.length})[0]||targetName||sourceName;
  const aliases=[...(target.aliases||[]),...(source.aliases||[]),targetName,sourceName].filter(x=>normalizedName(x)!==normalizedName(preferred));
...
function charactersShareIdentity(a,b){
  const ak=normalizedName(a.name),bk=normalizedName(b.name);if(!ak||!bk)return false;
  const aset=new Set([a.name,...(a.aliases||[])].map(normalizedName).filter(Boolean));const bset=new Set([b.name,...(b.aliases||[])].map(normalizedName).filter(Boolean));
  if([...aset].some(x=>bset.has(x)))return true;
  const short=ak.length<=bk.length?ak:bk,long=ak.length<=bk.length?bk:ak;
  if(long.startsWith(short)&&long.length-short.length>=2){const shortLabel=(ak.length<=bk.length?a.name:b.name).trim();return shortLabel.length>=2&&shortLabel.split(/\s+/).length===1}
  return false
}
...
function descriptorSimilarity(descriptor,c,p){
  ...
  const descriptorTerms=descriptor.replace(/^(?:ชาย|หญิง|เด็กชาย|เด็กหญิง|ลูก|ท่าน|องค์)/,'').match(/[ก-ฮ]{2,}/g)||[];
  for(const term of descriptorTerms)if(profile.includes(term))score+=45;
  ...
  for(const ch of novelEntries(p)){const text=String(ch.text||'');const di=text.indexOf(descriptor);if(di<0)continue;for(const n of names){const ni=text.indexOf(n);if(ni>=0&&Math.abs(ni-di)<=220)score+=20}}
  return score
}
function inferDescriptorAliases(p){
  let changed=0;const allText=novelEntries(p).map(x=>x.text||'').join('\n');
  for(const descriptor of DESCRIPTOR_HINTS){if(!allText.includes(descriptor))continue;if(isKnownOrEmbeddedCharacter(descriptor,p))continue;
    const ranked=(p.characters||[]).map(c=>({c,score:descriptorSimilarity(descriptor,c,p)})).sort((a,b)=>b.score-a.score);
    if(ranked[0]&&ranked[0].score>=55&&(!ranked[1]||ranked[0].score-ranked[1].score>=15)){ranked[0].c.aliases=uniqueTextValues([...(ranked[0].c.aliases||[]),descriptor]);ranked[0].c.updatedAt=now();changed++}
  }
  return changed
}
function reconcileCharacterIdentity(p){const mergedBefore=mergeDuplicateCharacters(p);const full=inferFullNamesFromNovel(p);const descriptors=inferDescriptorAliases(p);const mergedAfter=mergeDuplicateCharacters(p);return {merged:mergedBefore+mergedAfter,full,descriptors}}
...
function isKnownOrEmbeddedCharacter(name,p){const key=normalizedName(cleanCandidateName(name));if(!key)return true;for(const known of knownCharacterNames(p)){const k=normalizedName(known);if(!k)continue;if(key===k)return true;if(key.startsWith(k)&&key.length-k.length<=12&&THAI_ACTION_SUFFIXES.some(v=>key===k+normalizedName(v)))return true;if(k.startsWith(key)&&k.length-key.length<=18)return true}return false}
function plausibleNovelCandidate(name){const n=cleanCandidateName(name),low=n.toLowerCase();if(!n||n.length<2||n.length>24)return false;if(CANDIDATE_STOP_WORDS.has(n)||CANDIDATE_STOP_WORDS.has(low))return false;if(/\s{2,}|\d|[A-Za-z]{2,}|(?:บท|ตอน)ที่|DATABASE|CANON|ARC|PART|STATUS/i.test(n))return false;if(THAI_NON_NAME_PREFIXES.some(v=>n===v||n.startsWith(v)))return false;if(THAI_ACTION_SUFFIXES.some(v=>n===v))return false;if(/[!?…:;]|\b(?:ครับ|ค่ะ|คะ|นะ|สิ|เลย|แล้ว)$/u.test(n))return false;const thai=(n.match(/[ก-ฮ]/g)||[]).length;if(thai<2)return false;const words=n.split(/\s+/);if(words.length>3)return false;return true}
...
async function scanNovelCharacters(){if(!ensureProject())return;const p=project();p.characterCandidates=[];const reconciled=reconcileCharacterIdentity(p);const found=discoverCharacterCandidates(p);p.characterCandidates=found;activity('character',`วิเคราะห์ตัวละครจากนิยาย: รวมซ้ำ ${reconciled.merged} · เติมชื่อเต็ม ${reconciled.full} · เชื่อมชื่อเรียก ${reconciled.descriptors} · รอตรวจ ${found.length} (V31)`);await save();toast(`จัดระเบียบตัวละครแล้ว: รวม ${reconciled.merged} · ชื่อเต็ม ${reconciled.full} · ชื่อเรียก ${reconciled.descriptors} · รอตรวจ ${found.length}`)}
