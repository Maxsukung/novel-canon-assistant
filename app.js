...
async function scanNovelCharacters(){if(!ensureProject())return;const p=project();p.characterCandidates=[];const found=discoverCharacterCandidates(p);p.characterCandidates=found;activity('character',`วิเคราะห์ตัวละครจากนิยาย พบ ${found.length} รายการรอตรวจ (V30 โหมดแม่นยำสูง)`);await save();toast(found.length?`พบชื่อใหม่ที่น่าเชื่อถือ ${found.length} รายการ`:'ไม่พบชื่อใหม่ที่มีหลักฐานเพียงพอ')}
