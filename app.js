...
async function getPdfJs(){
  if(!pdfjsLib){
    pdfjsLib=await import('./pdf.legacy.min.mjs?v=14');
...
function pdfItemsToLines(items){
  ...
  for(const item of items||[]){
    let str=cleanPdfGlyphs(item.str);
    ...
    let row=rows.find(r=>Math.abs(r.y-y)<=Math.max(2,h*.22));
    if(!row){row={y,h,items:[]};rows.push(row)}
    row.items.push({str,x,width:Number(item.width||0),h});
  }
  ...
  return rows.map(row=>{
    ...
    for(const it of row.items){
      ...
      const threshold=Math.max(1.8,Math.min(prev?.h||it.h,it.h)*.28);
      const needsSpace=prev&&gap>threshold&&!/[\s([{\-–—/]$/.test(out)&&! /^[,.;:!?)}\]ฯๆ]/.test(it.str);
      out+=(needsSpace?' ':'')+it.str;
      prev=it;
    }
    return repairThaiText(cleanPdfGlyphs(out)).replace(/[ \t]+/g,' ').trim();
  }).filter(Boolean);
}
function linesToReadableText(lines){
  const out=[];let para='';
  const flush=()=>{if(para.trim()){out.push(para.trim());para=''}};
  const isDivider=s=>/^[-━═_]{5,}$/.test(s);
  const isHeading=s=>/^(?:บท|ตอน)ที่\s*[๐-๙0-9]+\s*[:：]/.test(s)||/^(?:หลายวันก่อนหน้านั้น|อำเภอ|จังหวัด|จบบท|จบตอน)/.test(s);
  for(let i=0;i<lines.length;i++){
    let line=cleanPdfGlyphs(lines[i]).trim();
    if(!line)continue;
    if(isDivider(line)){flush();out.push('━━━━━━━━━━━━━━━━━━━━');continue}
    if(isHeading(line)){flush();out.push(line);continue}
    const next=lines[i+1]||'';
    const ends=/[.!?…ฯ”"')\]]$/.test(line);
    para+=(para?' ':'')+line;
    if(ends||!next||isDivider(next)||isHeading(next)||para.length>1100)flush();
  }
  flush();
  return out.join('\n\n').replace(/\n{3,}/g,'\n\n').trim();
}
async function extractPdf(file){
  ...
  for(let n=1;n<=pdf.numPages;n++){
    ...
    const text=linesToReadableText(lines);
    pages.push({page:n,text});
  }
  const combined=pages.map(x=>`[หน้า ${x.page}]\n${x.text}`).join('\n\n');
  return {text:combined,pages,pageCount:pdf.numPages,extractionVersion:14};
}
...
function renderNovelReader(doc){
  ...
  $('novelReaderBody').innerHTML=`<article class="novel-reading-sheet"><h1 class="novel-reading-title">${esc(chapterDisplayTitle(doc))}</h1><div class="novel-reading-prose">${blocks.map(block=>{
    if(/^━━━━━━━━/.test(block))return '<div class="scene-divider">◆ ◆ ◆</div>';
    if(/^(?:บท|ตอน)ที่\s*[๐-๙0-9]+\s*[:：]/.test(block))return `<h2>${esc(block)}</h2>`;
    if(block.length<90&&!/[.!?…ฯ]$/.test(block)&&/^(?:หลายวันก่อนหน้านั้น|อำเภอ|จังหวัด|ตลาด|เส้นทาง|ลำห้วย)/.test(block))return `<h3>${esc(block)}</h3>`;
