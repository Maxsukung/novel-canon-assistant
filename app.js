...
function inferDescriptorAliases(p){
  ...
  for(const descriptor of DESCRIPTOR_HINTS){if(!allText.includes(descriptor))continue;if(isKnownOrEmbeddedCharacter(descriptor,p))continue;
    ...
    if(ranked[0]&&ranked[0].score>=70&&(!ranked[1]||ranked[0].score-ranked[1].score>=20)){ranked[0].c.aliases=uniqueTextValues([...(ranked[0].c.aliases||[]),descriptor]);ranked[0].c.updatedAt=now();changed++}
  }
  return changed
}
